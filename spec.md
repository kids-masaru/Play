...
# Claude予測参加・モデル別フィードバック学習・Gemma再学習計画（2026-09-01）

## 概要

- Claude Codeを、APIキーを使わないローカルCLI実行方式でボートレースとtotoの予測比較へ追加する。
- Codex・Claude・Gemini・Grokが、各モデル自身の確定済み予測と外れ方を次回予測へ利用できる共通のフィードバックRAG基盤を整備する。
- 「結果を保存するだけ」と「次回予測に結果を利用すること」を分け、モデル・戦略バージョンごとの学習材料と成績を追跡可能にする。
- Gemmaは現行モデルを上書きせず、新データで候補モデルを作成し、未学習期間の成績が改善した候補だけを採用する。
- 主な利用者は本システムの運用者本人。ダッシュボードとLINEで、モデルごとの現在成績と改善傾向を比較する。

## 現状認識

- Codexのボートレース予測とtoto予測は、確定済みの過去成績・直近ミスをプロンプトに渡すRAG型の仕組みを既に持つ。これはモデル本体の重みを更新する再学習ではない。
- Gemini・Grokは予測履歴を保存しているが、ボートレースでは自分の過去の外れ方を次回プロンプトへ十分に戻していない。
- totoのGeminiは統計モデルの情報を毎回利用するが、Gemini自身の過去予測に対するフィードバックは未接続。
- Gemmaの3モデルはQLoRAで学習済みだが、日次予測時は固定モデルによる推論のみで、自動再学習は行っていない。
- LightGBMは月曜日に再学習され、旧モデルより検証精度が良い場合だけ更新される。今回のGemma再学習とは別系統として維持する。

## 技術スタック

- Python 3 / pandas / 既存のCSV・JSONデータ処理
- Claude Code CLI（Claude Maxのログイン済み認証、日次実行はSonnet相当）
- Codex CLI、Gemini API、xAI API、Ollama、既存LightGBM
- React / Vite / Rechartsの既存ダッシュボード
- Windowsタスクスケジューラ、GitHub Actions、GitHub Pages、LINE Messaging API

## 機能要件

### 1. Claude Codeによるボートレース予測

- 当日分の全対象レースを可能な限り1回のClaude Code呼び出しへまとめる。
- Claude Codeのツール利用を無効化し、入力データだけを使う非対話・構造化JSON出力とする。
- 画面・LINE上の表示名は `Claude`、内部の実行方式・戦略識別子にはClaude Codeであることを保持する。
- 各レースの買い目件数、組み合わせ形式、重複禁止などは既存のCodex予測と同じ検証を行う。
- 過去レースへの後付け予測は行わず、実装後の次回未予測日から成績を蓄積する。
- Claude固有の予測履歴を保存し、確定後はClaude自身のフィードバック材料へ変換する。

### 2. Claude Codeによるtoto予測

- 販売中かつ未予測の回だけを対象とし、その回の全試合を可能な限り1回のClaude Code呼び出しで予測する。
- 既存の統計値、チーム状況、Claude自身の確定済み成績・直近ミスを入力する。
- 過去回への後付け予測は行わない。
- toto画面の予測比較、成績比較、試合詳細、収支シミュレーションへClaudeを追加する。
- 既存の統計・Gemini・Codex・ユーザー予測と同じ採点条件で比較する。

### 3. 共通フィードバックRAG

- 共通化するのは「確定済み履歴の抽出・集計・類似例の選定・プロンプト用要約・利用件数の記録」とする。
- 共通の過去レース結果・統計は全モデルが参照できる。
- 自己評価部分はモデルごとに分離し、GeminiはGemini、GrokはGrok、CodexはCodex、ClaudeはClaude自身の予測結果だけを参照する。
- 他モデルの予測や正解率を、あるモデルのフィードバックとして混ぜない。比較の公平性と原因追跡性を維持する。
- 対象レース・対象回より後の結果を参照しない時系列ガードを必須とする。
- 最低限、次の情報をモデル別に生成する。
  - 確定済み予測数、的中数、的中率、投資額、払戻額、ROI
  - 直近成績と全期間成績
  - 直近の外れ例と、選択・見落としの傾向
  - 条件が近い過去レースの要約
  - 使用した履歴件数、生成日時、戦略バージョン
- 初期実装は既存のCSV・JSONを用いる構造化検索とし、ベクトルDB導入は必須にしない。データ量や検索品質に問題が出た場合に後続候補とする。

### 4. 実行オーケストレーションと障害分離

- ボートレースの日次更新とtoto週次更新の両方へClaude処理を追加する。
- Claudeの認証切れ、利用上限、タイムアウト、JSON不正が発生しても、他モデルの予測・採点・ダッシュボード生成・LINE通知は継続する。
- 失敗時は不完全な予測ファイルを本番採用せず、ログへモデル名・対象日または対象回・原因を残す。
- 再実行時は正常に保存済みの対象を再生成せず、未完了分だけを処理できるようにする。
- Claude Codeの利用回数を抑えるため、ボートレースは原則1日1バッチ、totoは原則1対象回1バッチとする。

### 5. ダッシュボード・LINE表示

- ボートレースのモデル比較、ランキング、比較グラフ、過去成績へClaudeを追加する。
- Claudeのアイコンは既存のClaudeブランド表現と統一する。
- 基本Gemmaは従来どおりダッシュボードとLINE比較から除外したままとする。
- LINEのモデルランキングは既存の簡潔な2行表示を維持し、Claudeを同じ条件で自動的に含める。
- toto画面はスマホ・PCの両方でClaude追加後も表やカードが崩れないようにする。
- モデルごとに、参照したフィードバック件数と戦略バージョンをデータとして保持する。通常画面を混雑させる場合は常時表示せず、診断用データで確認可能にする。

### 6. Gemma候補モデルの再学習

- Claude・共通RAG対応とは別フェーズで実施する。
- 原則として、実装後に新しく確定した予測が300〜500件以上たまってから候補モデルを作成する。
- 教師データの対象期間、件数、生成方法、重複除去、リーク防止条件を記録する。
- 現行モデル・アダプター・Ollamaモデルは上書きせず、日付とデータ版を含む別名で候補を保存する。
- 現行モデルと候補モデルを、学習に含めていない同一の将来期間で比較する。
- 採用判定は的中率だけでなく、ROI、最大ドローダウン、予測欠損率、JSON失敗率、対象件数を確認する。
- 小標本の偶然勝ちを避け、最低評価件数を満たし、主要指標が総合的に改善した場合だけ本番モデルを切り替える。
- 改善しない候補は不採用とし、現行モデルを維持する。

## 非機能要件

- **公平性**: 後付け予測を禁止し、すべてのモデルを同じ確定結果・投資条件で採点する。
- **未来情報防止**: 予測日時より後の結果をRAG・再学習・評価に混入させない。
- **再現性**: モデル名、実行方式、戦略バージョン、入力対象、生成日時、参照履歴件数を保存する。
- **安全性**: Claude Code実行時はツール・MCP・ブラウザ連携を無効化し、リポジトリ変更を許可しない。
- **継続運用**: 1モデルの失敗で全バッチを失敗させず、再試行可能な状態を残す。
- **コスト管理**: ClaudeはMax利用枠、Codexはログイン済みCLI、Gemini・Grokは既存API枠を利用し、呼び出しを対象単位でまとめる。
- **可観測性**: 成功・スキップ・失敗・不完全出力をログとメタデータで区別できること。
- **互換性**: 既存の自動実行、LINE、GitHub Pages公開、PC・スマホ表示を壊さない。

## 完成・受け入れ条件

### Claude参加フェーズ

- ボートレースの実対象日でClaude予測が生成され、全件が既定フォーマット検証に合格する。
- totoの販売中対象回でClaude予測が生成され、全試合分が既定フォーマット検証に合格する。
- 両ダッシュボードとLINE比較へClaudeが表示される。
- Claudeを意図的にスキップまたは失敗させても、他モデルと公開データの更新が完了する。
- 過去日・過去回にClaudeの後付け予測が作成されない。

### フィードバックRAGフェーズ

- 各モデルの入力に、自分自身の確定済みフィードバックだけが含まれることをテストで確認する。
- 対象日・対象回以降の結果が混入しないことを境界テストで確認する。
- 履歴が0件、少数、十分にある場合のすべてで予測処理が継続する。
- 生成物から、利用した履歴件数と戦略バージョンを追跡できる。

### Gemma再学習フェーズ

- 現行モデルを保持したまま候補モデルが作成される。
- 同一の未学習評価セットによる比較レポートが作成される。
- 採用基準を満たす場合のみ切り替え、満たさない場合は現行モデルを維持する。

## 実装予定ファイル

### ボートレース

- `generate_claude_predictions.py`（新規）
- `codex_learning.py` または共通フィードバックモジュール（責務を整理して拡張）
- `generate_gemini_predictions.py`
- `generate_grok_predictions.py`
- `generate_codex_predictions.py`
- `generate_battle_data.py`
...
- `toto/predict_claude.py`（新規）
- `toto/predict_gemini.py`
- `toto/predict_codex.py`
- `toto/generate_toto_data.py`
- `toto/run_toto_weekly.py`

### ダッシュボード

- `dashboard/src/Battle.jsx`
- `dashboard/src/ModelDashboard.jsx`
- `dashboard/src/Toto.jsx`
- `dashboard/src/App.jsx`
- `dashboard/public/daily_data/` 配下の生成JSON

### Gemma再学習

- `gemma_finetune/` 配下の教師データ生成・学習・評価スクリプト
- 候補モデル名、評価結果、採否記録を管理するメタデータ

## 外部連携

- Claude Code CLI: Claude Maxのログイン済み認証を使用。APIキー課金は使わないが、Maxプランの利用上限は消費する。
- Codex CLI: 既存のログイン済み認証を継続利用する。
- Gemini API / xAI API: 既存の設定を継続利用する。
- LINE Messaging API: 既存のモデル比較通知へClaudeを追加する。
- GitHub Actions / GitHub Pages: ダッシュボード公開フローは既存構成を維持する。

## やらないこと（今回のスコープ外）

- Claude APIキーを新規契約してAPI課金方式へ切り替えること。
- Claude・Codex・Gemini・Grokの基盤モデル本体を本システムからファインチューニングすること。
- 過去レース・過去toto回へClaude予測を後付けし、見かけ上の成績を作ること。
- 他モデルの正解や予測を教師として混ぜ、モデル別比較の独立性を失わせること。
- 初期段階からベクトルDBを導入すること。
- Gemma候補を評価せず自動的に現行モデルへ上書きすること。

## 実施順序

1. 共通データ契約・戦略バージョン・時系列ガードを定義する。
2. ボートレースへClaude Code予測を追加し、単独スモークテストを行う。
3. totoへClaude Code予測を追加し、単独スモークテストを行う。
4. 共通フィードバックRAGを実装し、Codex・Claude・Gemini・Grokへモデル別に接続する。
5. バッチの障害分離、再実行、ログ、LINE、ダッシュボードを統合する。
6. 構文・単体・データ整合性・未来情報混入防止・レスポンシブ表示を確認する。
7. 次回未予測分から本運用を開始し、新規確定データを蓄積する。
8. 300〜500件以上の新規確定データ蓄積後、Gemma候補再学習と比較評価を別フェーズで行う。

## 備考・制約

- RAG型フィードバックは「保存済みの学習材料を毎回プロンプトへ渡す」方式であり、モデルの重みが日々更新されるわけではない。
- Claude・Codex・Gemini・Grokの提供元が基盤モデルを更新する可能性はあるが、それはPlay固有データによる学習ではない。
- Claude Maxや各APIの利用上限・認証状態により、一時的にそのモデルだけ欠測する可能性がある。
- 現在の作業ツリーには自動生成データ等の未コミット変更があるため、実装時は対象コードと必要な生成物だけを選別してコミットする。
